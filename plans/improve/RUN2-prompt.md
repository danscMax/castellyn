...
- **База:** ветка `auto-loop/improve-0703-run2` (Фаза A уже начата в этом сеансе). **Пункт 18 (Дыра 1) УЖЕ СДЕЛАН** — коммит `41ef6a8` (durable sidecar + Syncthing + бэкап/restore + миграция), гейты зелёные. НЕ переделывать.
- **План:** `plans/improve/fix_plan-2026-07-03.json` + `plans/improve/master-plan-run2-redesign.md` (фазовое разбиение). Выполнить ОСТАТОК **ФАЗЫ A** — redesign-safe (все `approved:true`, решения в `decision_resolved`): **#5, 11, 15, 17, 19, 21c, 21e** + из **21d** только колонка utilization в **Профилях** (ProfilesTab). **НЕ делать** `10`, `13`, и чип-часть `21d` — они привязаны к верстке Сессий и вплавляются в herdr-редизайн (Фаза B). НЕ перезапускать Прогон 1 (Волны 1–3 кроме #5, 21a/21b) и уже готовый 18.
...
- **Порядок:** #5, 17, 15, 11, 19, 21c, 21e, 21d-Профили (18 уже сделан). Пункты, трогающие `SessionsTab.svelte` (11, 21c) — сериализовать (`⚠ same-file`), не параллелить.
