...
<tr>
<td colspan="2" align="center"><b>Environments · Среды · 环境</b><br><img src="docs/img/screenshot-environments.png" alt="Environments tab" /></td>
</tr>
<tr>
...
<tr>
<td colspan="2" align="center"><b>Subagents · Субагенты · 子代理</b><br><img src="docs/img/screenshot-subagents.png" alt="Subagents tab" /></td>
</tr>
<tr>
...
| | Tab | What it does |
|---|---|---|
| ▦ | **Sessions** | Spin up **parallel terminal sessions** (real PTY, xterm.js) and launch Claude Code or opencode in any profile + provider — several agents side by side in one window. A **quota‑aware launch advisor** recommends a profile + effort from live usage limits, with structured `low/medium/high/max` effort and model controls. |
...
| ⛶ | **Environments** | One canonical setup fanned out to **every AI‑coding harness** on the machine (Claude Code, OpenCode, Codex): per‑harness skills / providers / MCP / RTK coverage, one‑click **skill sharing**, and deploys of MCP servers, providers and rule files into OpenCode & Codex. |
...
| ✦ | **Subagents** | Author standalone Claude Code subagents (`~/.claude/agents/*.md`) in a GUI — frontmatter (name, description, model, tools) + system prompt, with a ready **Codex template** — fanned out to every profile via shared folders + Syncthing. |
...
Grab the latest build from **[Releases](https://github.com/danscMax/castellyn/releases/latest)** — a standalone `.exe`, an NSIS setup, or an MSI (Windows x64). No toolchain needed to just run it.

> **Requirements:** Windows 10/11 + **[PowerShell 7](https://aka.ms/powershell)** (`pwsh`). The maintenance scripts run under `pwsh`, which Windows does **not** ship by default (it only includes Windows PowerShell 5.1) — without it every action fails with “failed to launch pwsh”. WebView2 already ships with Windows.

> **SmartScreen:** release binaries are not Authenticode-signed yet, so the first launch may show “Windows protected your PC”. Click **More info → Run anyway**. If Defender quarantines the download, restore it and add an exclusion — or build from source (below).
...
> **Platform:** Windows‑first (PowerShell 7 / `pwsh` maintenance scripts, Windows Terminal, Task Scheduler).
...
| | Вкладка | Что делает |
|---|---|---|
| ▦ | **Сессии** | **Параллельные терминалы** (настоящий PTY, xterm.js): запуск Claude Code или opencode в любом профиле с выбранным провайдером — несколько агентов рядом в одном окне. **Советник запуска с учётом лимитов** подсказывает профиль и уровень усилия по свежим лимитам; структурные контролы усилия (`low/medium/high/max`) и модели. |
...
| ⛶ | **Среды** | Одна каноническая настройка раскатывается на **все AI‑агенты** машины (Claude Code, OpenCode, Codex): покрытие скиллов / провайдеров / MCP / RTK по каждой среде, **шаринг скиллов** в один клик и деплой MCP‑серверов, провайдеров и файлов правил в OpenCode и Codex. |
...
| ✦ | **Субагенты** | Создание автономных субагентов Claude Code (`~/.claude/agents/*.md`) в GUI — фронтматтер (имя, описание, модель, инструменты) + системный промпт, с готовым **шаблоном Codex** — раскатка на все профили через общие папки и Syncthing. |
...
Возьмите последнюю сборку в **[Releases](https://github.com/danscMax/castellyn/releases/latest)** — автономный `.exe`, установщик NSIS или MSI (Windows x64). Чтобы просто запустить, тулчейн не нужен.

> **Требования:** Windows 10/11 + **[PowerShell 7](https://aka.ms/powershell)** (`pwsh`). Обслуживающие скрипты выполняются под `pwsh`, который Windows **не** ставит по умолчанию (есть только Windows PowerShell 5.1) — без него любое действие падает с «failed to launch pwsh». WebView2 уже входит в Windows.

> **SmartScreen:** релизные бинарники пока не подписаны Authenticode, поэтому при первом запуске Windows может показать «Система Windows защитила ваш компьютер». Нажмите **Подробнее → Выполнить в любом случае**. Если Defender отправил файл в карантин — восстановите его и добавьте исключение, либо соберите из исходников (ниже).
...
> **Платформа:** Windows‑first (PowerShell 7 / `pwsh` скрипты обслуживания, Windows Terminal, планировщик Windows).
...
| | 标签页 | 作用 |
|---|---|---|
| ▦ | **会话** | 开启**并行终端会话**（真实 PTY、xterm.js），以任意配置和指定提供商启动 Claude Code 或 opencode——多个智能体在同一窗口中并排运行。内置**配额感知的启动顾问**，依据实时用量限额推荐配置与投入档位，并提供结构化的投入（`low/medium/high/max`）与模型控件。 |
...
| ⛶ | **环境** | 一套规范配置分发到本机**所有 AI 编码工具**（Claude Code、OpenCode、Codex）：按环境查看技能 / 提供商 / MCP / RTK 覆盖，一键**共享技能**，并将 MCP 服务器、提供商和规则文件部署到 OpenCode 与 Codex。 |
| 🧩 | **插件与技能** | 在可排序表格中管理 Claude Code 插件**与技能**：启用 / 停用 / 更新 / 删除、批量操作，并清晰区分**归属**（本地自有市场 / 第三方 / 默认）。 |
| ✦ | **子代理** | 在图形界面中编写独立的 Claude Code 子代理（`~/.claude/agents/*.md`）——frontmatter（名称、描述、模型、工具）+ 系统提示词，内置 **Codex 模板**——通过共享文件夹与 Syncthing 分发到所有配置。 |
...
从 **[Releases](https://github.com/danscMax/castellyn/releases/latest)** 获取最新构建——独立 `.exe`、NSIS 或 MSI 安装包（Windows x64）。只运行的话无需工具链。

> **要求：** Windows 10/11 + **[PowerShell 7](https://aka.ms/powershell)**（`pwsh`）。维护脚本在 `pwsh` 下运行，而 Windows **不会**默认安装它（仅自带 Windows PowerShell 5.1）——缺少它任何操作都会以「failed to launch pwsh」失败。WebView2 已随 Windows 提供。

> **SmartScreen：** 发布的二进制文件尚未进行 Authenticode 签名，首次启动时 Windows 可能提示「Windows 已保护你的电脑」。点击**更多信息 → 仍要运行**。若 Defender 隔离了下载文件，请还原并添加排除项，或从源码构建（见下文）。
...
> **平台：** 以 Windows 为先（PowerShell 7 / `pwsh` 维护脚本、Windows Terminal、任务计划程序）。
