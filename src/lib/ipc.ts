 ...
 * `ask` is the lifecycle hook's own account of what a blocked pane is waiting for (already
 * sanitised and length-capped there); hookless panes have none and fall back to `askPreview`. */
export type AgentStatusEvent = { id: string; state: string; claudeSessionId: string | null; spawnedAt?: number; hookIdle?: boolean; limitMenu?: boolean; ask?: string | null };
