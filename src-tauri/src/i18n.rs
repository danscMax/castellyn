...
const TABLE: &[(&str, [&str; 3])] = &[
    ...
    ("err.fs_read", ["чтение {path}: {e}", "reading {path}: {e}", "读取 {path}: {e}"]),
    ("err.fs_parse", ["разбор {path}: {e}", "parsing {path}: {e}", "解析 {path}: {e}"]),
    ("err.fs_rename", ["переименование {path}: {e}", "renaming {path}: {e}", "重命名 {path}: {e}"]),
    ...
    ("err.junction_conflict", ["{link} существует, но не указывает на {target} — уберите его вручную и повторите", "{link} exists but does not point to {target} — remove it manually and retry", "{link} 已存在但未指向 {target} — 请手动移除后重试"]),
    ("onb.not_on_path", ["`{name}` не найден в PATH", "`{name}` not found on PATH", "在 PATH 中未找到 `{name}`"]),
    ("onb.syncthing_cfg_missing", ["config.xml Syncthing не найден", "Syncthing config.xml not found", "未找到 Syncthing 的 config.xml"]),
    ("onb.tree_missing", ["{path} отсутствует — синхронизируйте дерево настроек или восстановите бэкап", "{path} missing — sync the settings tree or restore a backup", "{path} 缺失 — 请同步设置目录树或恢复备份"]),
    ("onb.missing_path", ["{path} отсутствует", "{path} missing", "{path} 缺失"]),
    ("onb.junction_wrong_target", ["указывает на {target}", "points to {target}", "指向 {target}"]),
    ("onb.junction_plain_dir", ["обычная папка, а не джанкшен", "a plain directory, not a junction", "是普通文件夹而非 junction"]),
    ("onb.junction_unreadable", ["цель джанкшена нечитаема", "junction target unreadable", "junction 目标不可读"]),
    ("onb.creds_missing", ["{path} отсутствует — восстановите бэкап или войдите один раз", "{path} missing — restore a backup or log in once", "{path} 缺失 — 请恢复备份或登录一次"]),
    ...
    // ── command errors that were still raw English (L3) ──────────────────────
    ("err.proxy_url_scheme", ["URL прокси должен начинаться с http://, https:// или socks5://", "proxy URL must start with http://, https:// or socks5://", "代理 URL 必须以 http://、https:// 或 socks5:// 开头"]),
    ("err.profiles_json_missing", ["profiles.json не найден", "profiles.json not found", "未找到 profiles.json"]),
    ("err.unknown_shared_folder", ["неизвестная общая папка: {f}", "unknown shared folder: {f}", "未知的共享文件夹: {f}"]),
    ("err.not_own_marketplace", ["{market} — не свой маркетплейс (источник не каталог)", "{market} is not an own (directory-source) marketplace", "{market} 不是自有（目录源）插件市场"]),
    ("err.installed_plugins_unreadable", ["installed_plugins.json не читается", "installed_plugins.json unreadable", "无法读取 installed_plugins.json"]),
    ("err.no_install_location", ["у маркетплейса {market} нет installLocation", "no installLocation for {market}", "插件市场 {market} 没有 installLocation"]),
    ("err.bump_script_missing", ["скрипт bump не найден: {script}", "bump script not found: {script}", "未找到 bump 脚本: {script}"]),
    ("err.keyring_save", ["не удалось сохранить учётные данные: {e}", "could not save the credential: {e}", "无法保存凭据: {e}"]),
    ("err.file_not_found", ["файл не найден: {path}", "file not found: {path}", "未找到文件: {path}"]),
    ("err.not_regular_file", ["это не обычный файл", "not a regular file", "不是普通文件"]),
    ("err.blocked_file_type", ["этот тип файла не открывается автоматически — открытие могло бы его запустить", "this file type is not opened automatically — opening it could run it", "该类型的文件不会自动打开 — 打开它可能会执行它"]),
    ("err.no_editor_for_type", ["не найден редактор для этого типа файла", "no editor found for this file type", "未找到适用于此文件类型的编辑器"]),
    ("err.not_sync_conflict", ["это не файл sync-conflict: {path}", "not a sync-conflict file: {path}", "这不是 sync-conflict 文件: {path}"]),
    ("err.path_outside_claude", ["отказано: {path} находится вне {root}", "refused: {path} is outside {root}", "已拒绝: {path} 位于 {root} 之外"]),
    ("err.mklink_failed", ["mklink /J не сработал (код выхода {code})", "mklink /J failed (exit {code})", "mklink /J 失败 (退出码 {code})"]),
    ("err.monitor_window_open", ["окно для этого монитора уже открыто", "the window for this monitor is already open", "该显示器的窗口已打开"]),
    ("err.monitor_out_of_range", ["монитора с таким номером нет", "no monitor with that index", "不存在该编号的显示器"]),
    ("err.recycle_aborted", ["удаление в корзину прервано", "recycling was aborted", "移入回收站的操作已中止"]),
    ("err.recycle_failed", ["не удалось удалить в корзину (SHFileOperation 0x{rc})", "could not move to the Recycle Bin (SHFileOperation 0x{rc})", "无法移入回收站 (SHFileOperation 0x{rc})"]),
    ...
    ("log.plugin_managed_blocked", ["Плагин {id} заблокирован политикой (managed-settings) — включение невозможно. Разблокируй в источнике и передеплой.", "Plugin {id} is blocked by managed policy — enable is impossible. Unblock it in the source and redeploy.", "插件 {id} 被 managed 策略阻止 — 无法启用。请在源中解除阻止并重新部署。"]),
    ("log.plugin_failed_n", ["Провалов: {n} — детали в логе выше.", "{n} failure(s) — details in the log above.", "{n} 个失败 — 详见上方日志。"]),
    ("err.plugin_not_blocked", ["{id}: в managed-источнике нет явной блокировки", "{id}: no explicit block in the managed source", "{id}: managed 源中没有显式阻止"]),
